...
def get_current_mask_color():
    ...
    if st.session_state.mask_color_type == "预设颜色":
        from . import PRESET_COLORS  # 从模块导入
...
with st.sidebar:
    ...
    if safe_update_state('product_size', product_size):
        pass
    ...
    if safe_update_state('dark_mask_enabled', dark_mask_enabled):
        pass
    ...
    if dark_mask_enabled:
        ...
        if safe_update_state('mask_opacity', mask_opacity):
            pass
        ...
        with col_type1:
            if st.button("预设颜色", use_container_width=True,
                        type="primary" if st.session_state.mask_color_type == '预设颜色' else "secondary",
                        key="preset_color_btn"):
                if safe_update_state('mask_color_type', "预设颜色"):
                    st.rerun()
        
        with col_type2:
            if st.button("自定义", use_container_width=True,
                        type="primary" if st.session_state.mask_color_type == '自定义颜色' else "secondary",
                        key="custom_color_btn"):
                if safe_update_state('mask_color_type', "自定义颜色"):
                    st.rerun()
        ...
        if st.session_state.mask_color_type == "预设颜色":
            ...
            if safe_update_state('mask_preset_color', selected_color):
            ...
            if safe_update_state('mask_custom_color', custom_color):
    ...
    if safe_update_state('output_size', output_size):
        pass
    ...
    if safe_update_state('output_format', output_format):
        pass
...
with tab1:
    ...
    with col1:
        ...
        if bg_source == "上传图片":
            ...
            if bg_files:
                ...
                for idx, file in enumerate(bg_files[:9]):
            ...
            with search_col2:
                if st.button("搜索", type="primary", use_container_width=True, key="search_unsplash"):
                    if search_query and safe_update_state('unsplash_search_query', search_query):
            ...
            # 处理搜索和翻页
            need_search = st.session_state.unsplash_search_trigger
            
            if need_search and unsplash_api.access_key:
            ...
            if st.session_state.unsplash_photos:
                ...
                total_results = st.session_state.unsplash_total_results
                
                # 显示分页信息
                if total_results > 0:
                    st.info(f"📊 找到 {total_results} 张图片 - 第 {current_page}/{total_pages} 页")
                ...
                for idx, photo in enumerate(photos):
                    with cols[idx % 4]:
                        ...
                        <div class="unsplash-card">
                            <div class="unsplash-img-container" style="background-image: url('{img_url}');"></div>
                        </div>
                        """, unsafe_allow_html=True)
                        ...
                        if selected_bg and isinstance(selected_bg, dict):
                            is_selected = (selected_bg.get('page') == current_page and 
                                          selected_bg.get('idx') == idx)
                        
                        # 选择按钮
                        if st.button(
                            "选择背景图",
                            key=f"select_bg_{current_page}_{idx}",
                            type="primary" if is_selected else "secondary",
                            use_container_width=True
                        ):
                            # 设置选择状态
                            st.session_state.unsplash_selected_bg = {
                                'page': current_page,
                                'idx': idx,
                                'img_url': img_url,
                                'photo': photo
                            }
                            st.session_state.background_download_task = {
                                'url': photo.get("urls", {}).get("regular") or img_url,
                                'page': current_page,
                                'idx': idx
                            }
                            st.success("背景图已选择！")
            ...
            # 处理背景图下载任务
            if st.session_state.background_download_task:
                task = st.session_state.background_download_task
                with st.spinner("下载背景图中..."):
                    img = unsplash_api.download_photo(task['url'])
                    if img:
                        # 创建模拟文件对象
                        class MockFile:
                        ...
                        mock_file = MockFile(img, task['page'], task['idx'])
                        st.session_state.unsplash_selected_bg_file = mock_file
                        st.session_state.background_download_task = None
                        st.success("背景图下载完成！")
                    else:
                        st.error("背景图下载失败")
                        st.session_state.background_download_task = None
    ...
    with col2:
        ...
        if product_files:
            ...
            # 显示预览
            st.markdown("预览（最多显示9张）")
            preview_cols = st.columns(3)
            for idx, file in enumerate(product_files[:9]):
                with preview_cols[idx % 3]:
                    img = Image.open(file)
                    img.thumbnail((120, 120), Image.Resampling.LANCZOS)
                    st.image(img, caption=file.name[:10] + "...", use_column_width=True)
...
if process_button:
    ...
    # 获取Logo图像对象
    logo_path = "logos/black_logo.png" if st.session_state.logo_color == "黑色Logo" else "logos/white_logo.png"
    logo_to_use = None
        ...
        for i, bg_file in enumerate(bg_files_combined):
            ...
            for j, product_file in enumerate(product_files):
                ...
                if result:
                    ...
                    output_filename = f"{bg_name}_{os.path.splitext(product_file.name)[0]}.{st.session_state.output_format.lower()}"
...
# ==================== 其他标签页保持不变 ====================
# 这里只修复了tab1的问题，tab2和tab3保持原有代码
# 如果需要，我可以继续优化tab2和tab3
