...
# 页面标题
st.markdown('h1 class=main-header🎨 骏泰素材工作台h1', unsafe_allow_html=True)
st.markdown(---)
...
class UnsplashAPI
    def __init__(self)
        ...
        self.base_url = httpsapi.unsplash.com
    ...
    def search_photos(self, query, page=1, per_page=12)
        ...
        url = f{self.base_url}searchphotos
        headers = {Authorization fClient-ID {self.access_key}}
        params = {
            query query,
            page page,
            per_page per_page,
        ...
        try
            ...
            if response.status_code == 200
                ...
                st.error(fUnsplash API错误 {response.status_code})
                return [], 0, 0
        except Exception as e
            st.error(fUnsplash API请求失败 {e})
            return [], 0, 0
    ...
    try
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file
            ...
            for i in range(num_copies)
                ...
                img_buffer = BytesIO()
                # 根据原格式保存，保持质量
                if uploaded_file.type in ['imagejpeg', 'imagejpg']
                ...
                img_buffer.seek(0)
                # 生成文件名：原名称_序号
                file_name = f{os.path.splitext(uploaded_file.name)[0]}_modified_{i+103d}{ext}
...
def remove_random_frames(input_video_path, output_video_path, progress_bar=None, status_text=None)
    ...
    if not os.path.exists(input_video_path)
        raise FileNotFoundError(f找不到输入视频文件 '{input_video_path}')
    ...
    # 更新状态
    if status_text
        status_text.text(f将删除第 {frames_to_remove[0]} 帧和第 {frames_to_remove[1]} 帧)
    ...
    try
        ...
        st.warning(f音频处理出现异常，将继续处理视频（可能无音频） {e})
        has_audio = False
    ...
    if has_audio
        ...
        try
            ...
            st.warning(f音视频合并失败，将输出无音频视频 {e})
            # 如果合并失败，则将无音频视频作为输出
            if os.path.exists('temp_video_noaudio.mp4')
...
def generate_product_content(product_name, platform)
    ...
    # 确保属性词数量
    attribute_text = n.join(attributes)
...
def create_zip_from_images(images, original_names, output_format='PNG')
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file
        for i, (img, original_name) in enumerate(zip(images, original_names))
            ...
            filename = f{name_without_ext}_with_logo_{i+103d}{ext}
...
with st.sidebar
    ...
    # 1. Logo设置
    st.markdown('div class=settings-title🖼️ Logo设置div', unsafe_allow_html=True)
    ...
    # 2. 产品图设置
    st.markdown('div class=settings-title📐 产品图设置div', unsafe_allow_html=True)
    ...
    # 3. 输出设置
    st.markdown('div class=settings-title📦 输出设置div', unsafe_allow_html=True)
...
with tab1
    ...
    div class=highlight-box
        p上传合适的背景图或unsplash图库中搜索，再上传透明产品图，左侧合成带LOGO产品图p
    div
                    ...
                    with st.spinner(f'正在搜索{st.session_state.unsplash_search_query}...')
                        ...
                        if photos
                            ...
                            if st.session_state.unsplash_current_page == 1
                                st.success(f找到 {total_results} 张图片，共{total_pages}页)
                        else
                            if total_results == 0
                                st.warning(f未找到与'{st.session_state.unsplash_search_query}'相关的图片)
                            else
                        ...
                        if idx  len(photos)
                            with columns[col]
                                ...
                                if img_url
                                    ...
                                    btn_kwargs = {
                                        key fselect_{current_page}_{idx},
                                        use_container_width True,
                                        help 
                                    }
                                            ...
                                            if img
                                                class MockFile
                                                    def __init__(self, img, idx)
                                                        self.name = funsplash_bg_{current_page}_{idx}.jpg
                                                        self.type = imagejpeg
                                                        self.image = img
                                                        self.idx = idx  # 记录索引
    ...
    with col2
        ...
        if product_files
            ...
            st.success(f已上传 {product_count} 张产品图)
    ...
    if bg_files_combined and product_files
        ...
        st.info(f准备合成 {len(bg_files_combined)} 张背景图 × {len(product_files)} 张产品图 = {total_combinations} 张合成图)
...
with tab2
    ...
    st.markdown(
    div class=highlight-box
        p通过微调图片像素，生成大量数据层不同的相似图片，可用于应对平台的重复检测。p
    div
    , unsafe_allow_html=True)
    ...
    with col_left
        ...
        if uploaded_file
            ...
            # 显示调整后的图片
            st.image(display_img, caption=f原图 {uploaded_file.name}, width=display_width)
            
            # 显示图片信息
            st.caption(f尺寸 {img.width} × {img.height} 像素  格式 {uploaded_file.type})
                        ...
                        if preview_images
                            ...
                            for idx, preview_img in enumerate(preview_images)
                                with preview_cols[idx]
                                    ...
                                    st.caption(f尺寸 {preview_img.width} × {preview_img.height})
...
with tab3
    ...
    st.markdown(
    div class=highlight-box
        p通过随机删除视频中的两帧，生成内容相似但数据不同的新视频，可用于应对平台的重复检测。p
    div
    , unsafe_allow_html=True)
            ...
            try
                ...
                if cap.isOpened()
                    ...
                    div class=video-info-card
                        div class=video-info-title📊 视频详情div
                        div class=video-info-text
                            • 文件名 {video_file.name}br
                            • 分辨率 {width} × {height}br
                            • 帧率 {fps.2f} FPSbr
                            • 总帧数 {total_frames} 帧br
                            • 时长 {duration.2f} 秒br
                            • 文件大小 {video_file.size  (10241024).2f} MB
                        div
                    div
                    , unsafe_allow_html=True)
            ...
            if st.button(🎬 开始视频抽帧处理, type=primary, use_container_width=True, key=process_video)
                with st.spinner('正在处理视频...')
                    ...
                    try
                        ...
                        st.markdown(f
                        div class=video-info-card
                            div class=video-info-title✅ 处理成功div
                            div class=video-info-text
                                • 删除的帧 第 {frames_removed[0]} 帧和第 {frames_removed[1]} 帧br
                                • 原视频帧数 {video_info['total_frames']} 帧br
                                • 新视频帧数 {saved_frames} 帧br
                                • 删除帧数 {video_info['total_frames'] - saved_frames} 帧br
                                • 分辨率 {video_info['width']} × {video_info['height']}br
                                • 帧率 {video_info['fps'].2f} FPSbr
...
# 标签页4：AI文案（暂不可用）
with tab4
    st.header(📝 AI文案 - 阿里巴巴MIC平台)
    st.markdown(
    div class=highlight-box
        pb功能说明：b根据选择的产品，自动生成适用于阿里巴巴和国际站(MIC)的英文产品标题、关键词和属性词。p
        ul
            li标题长度：8-12个单词，85-128个字符，首字母大写，介词小写li
            li关键词：包含短尾核心词和长尾复合词li
        ul
    div
    , unsafe_allow_html=True)
    ...
    with col_preview
        if st.session_state.generated_titles
            ...
            # 标题部分
            st.markdown('div class=section-title📝 10个产品标题div', unsafe_allow_html=True)
            st.markdown(复制说明： 以下标题可直接复制到阿里MIC平台的产品标题字段)
            
            # 创建可复制的文本框
            titles_text = n.join(st.session_state.generated_titles)
            st.text_area(
                产品标题 (共10个),
                titles_text,
                height=200,
            ...
            # 关键词部分
            st.markdown('div class=section-title🔑 10个关键词div', unsafe_allow_html=True)
            st.markdown(包含： 短尾核心词 + 长尾复合词)
            
            keywords_text = n.join(st.session_state.generated_keywords)
            st.text_area(
                关键词列表,
                keywords_text,
                height=150,
            ...
            # 属性词部分
            st.markdown('div class=section-title🏷️ 10个属性词div', unsafe_allow_html=True)
...
with tab5
    ...
    st.markdown(
    div class=highlight-box
        p为单张图片添加Logo水印，支持自定义Logo位置、大小和透明度。p
    div
    , unsafe_allow_html=True)
    ...
    with col_left
        ...
        if uploaded_image
            ...
            st.markdown(原图信息)
            st.caption(f文件名 {uploaded_image.name})
            st.caption(f尺寸 {img.width} × {img.height} 像素)
            st.caption(f格式 {uploaded_image.type})
    ...
    with col_middle
        ...
        st.markdown(f当前值 {int(opacity255100)}%)
    ...
    with col_right
        ...
        if uploaded_image
            ...
            if st.session_state.logo_adder_logo_color == 黑色Logo
                logo_path = logosb_logo.png
            else
                logo_path = logosw_logo.png
            ...
            if not logo_exists
                ...
                if processed_result
                    ...
                    # 显示Logo位置信息
                    st.caption(f原图尺寸 {original_width} × {original_height} 像素)
                    st.caption(fLogo位置 X={logo_x}px, Y={logo_y}px  大小 {logo_width}px × {logo_width}px  透明度 {int(st.session_state.logo_adder_logo_opacity255100)}%)
                    ...
                    st.info(f文件大小 {file_size_kb.1f} KB  格式 JPG  质量 95%)
            ...
            div style=text-align center; padding 2rem; color #666;
                h4👈 请先在左侧上传图片h4
                p上传图片后，可以调整Logo设置并实时预览效果p
                p支持单张图片处理，直接下载JPG格式p
            div
            , unsafe_allow_html=True)
            st.markdown('div', unsafe_allow_html=True)
...
if process_button
    ...
    if logo_color == 黑色Logo
        logo_path = logosblack_logo.png
    else
        logo_path = logoswhite_logo.png
    ...
    with tempfile.TemporaryDirectory() as tmpdir
        ...
        for i, bg_file in enumerate(bg_files_combined)
            ...
            for j, product_file in enumerate(product_files)
                ...
                progress_bar.progress(progress)
                status_text.text(f正在处理 {processed}{total} ({progress100.1f}%))
        ...
        st.download_button(
            ...
            data=zip_buffer,
            file_name=f产品图合成_{output_size}px_{output_format.lower()}.zip,
            mime=applicationzip,
            use_container_width=True
        )
        ...
        if output_files
            ...
            # 显示图片数量信息
            st.write(f共 {len(output_files)} 张合成图片，显示前 {total_previews} 张预览)
